class PaymentJob < ApplicationJob
  ...
  def perform(payment)
    ...
    if result_code == 0 # Payment Successful
      ...
      @transaction.update!(payment_details: payload)
      ...
      if Radprofile::CreateUser.call(
        ...
        password: generated_password, # Pass plain for hashing in service
        group_name: subscription_plan
      )
        transaction.update!(
          username: generated_username,
          password_digest: Digest::SHA256.hexdigest(generated_password) # Store a simple hash for internal record
        )
      ...
      @transaction.update!(status: :failed, payment_details: payload)
